...
export const syncUtils = (
  ...
  const syncItemByID = async (id: string, cbs: SubscriptionCallbacks = {}) => {
    console.log('sync-utils/ syncingClient.ts /  syncItemByID   cbs =', cbs )
    startSync();
    const logger = createLogger(cbs);
    console.log('sync-utils/ syncingClient.ts /  syncItemByID  id  =', id)
    const shopifyItem = await fetchItemById(id, true);
    console.log('sync-utils/ syncingClient.ts /  syncItemByID  shopifyItem  =', shopifyItem)
    if (!shopifyItem) {
      onFetchComplete();
    ...
    if (shopifyItem.__typename === 'Product') {
      ...
      logger.logLinked(
        syncResult.operation.sanityDocument,
        linkOperation.pairs,
      );
      onComplete();
      return;
    ...
    if (shopifyItem.__typename === 'Collection') {
      console.log('sync-utils/syncingClient/  COLLECTION  shopifyItem  =', shopifyItem)
  ...
  const deleteArchivedDocuments = async () => {
    const query =
      '*[_type == "shopifyProduct" || _type == "shopifyCollection"]';
    await sanityClient
      .patch({ query })
...
export const createSyncingClient = ({
  ...
  console.log('sync-utils/ syncingClient.ts /  createSyncingClient  sanityClient  =', sanityClient)

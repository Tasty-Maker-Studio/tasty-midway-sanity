...
import { createShopifyClient, syncUtils, KEYS_ID } from '../sync-utils';
...
export const useSaneContext = () => {
  ...
  if (!ctx)
    throw new Error('useSaneContext must be used within a SaneProvider');
  return ctx;
};
...
> {
  ...
  public async componentDidMount(): Promise<void> {
    ...
    await this.createSyncingClient(shopifySecrets);
  }
  ...
  public saveSecrets = async (secrets: ShopifySecrets): Promise<void> => {
    ...
    await this.createSyncingClient(secrets);
  };

  public clearSecrets = async (): Promise<void> => {
    if (this.syncingClient) {
      await this.syncingClient.clearSecrets();
    }
  };

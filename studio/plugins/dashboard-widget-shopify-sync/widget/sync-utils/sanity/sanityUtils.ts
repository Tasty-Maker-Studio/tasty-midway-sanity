...
export interface SanityCache {
  getById: (id: string) => SanityShopifyTabDocument | null;
  getByHandle: (handle: string, type: string) => SanityShopifyTabDocument | null;
  getByShopifyId: (id: string) => SanityShopifyTabDocument | null;
  set: (doc: SanityShopifyTabDocument) => void;
}

const createCache = (): SanityCache => {
  const ids: StringCache<SanityShopifyTabDocument> = {};
  const collectionHandles: StringCache<SanityShopifyTabDocument> = {};
  const productHandles: StringCache<SanityShopifyTabDocument> = {};
  const shopifyIds: StringCache<SanityShopifyTabDocument> = {};
  ...
  const getByHandle = (handle: string, type: string) => {
    const handles =
      type === 'collection'
        ...
        : null;
    if (!handles)
      throw new Error(`There is no handle cache for type "${type}"`);
  ...
  const set = (doc?: SanityShopifyTabDocument) => {
    if (!doc) return;
    ids[doc._id] = doc;
    const handles =
    ...
    handles[doc.content.main.handle] = doc;
    if (doc.shopifyId){
    } shopifyIds[doc.shopifyId] = doc;
  };

...
import { SanityShopifyProductTabDocument, Collection, Product } from '../types';
...
export const mergePaginatedResults = <NodeType>(
  ...
  if (!p1.pageInfo || !p2.pageInfo)
    throw new Error('Page info was not suplied');
...
export const prepareSourceData = <T extends Product | Collection>(item: T) => {
  if (isShopifyProduct(item)) {
    ...
    return {
      ...
      images: {
        ...item.images,
        edges: definitely(item.images).map(({ cursor, node }) => {
...
export const stringHtmlToBlocks = (html: string): BlockText[] => {
  ...
  // may need to use JSDOM for lamda
